<template>
  ...
  export default {
    ...
    async created() {
      ...
      try {
        this.loading = true;
        console.log('Fetching completed works for user:', this.user.uid);
        ...
        if (error.response?.data) {
          this.errorDetails = `${error.response.data.error}: ${error.response.data.details || ''}`;
        } else {
    ...
    methods: {
      async markAsIncomplete(work) {
        try {
          console.log('Marking work as incomplete:', work.id);
          await axios.delete(`http://localhost:8001/completed-works/${this.user.uid}/${work.id}`);
          
          console.log('Adding work back to likes');
